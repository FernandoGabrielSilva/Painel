...
install_neofetch() {
  case "$1" in
    apt)
      echo "Instalando Neofetch via apt..."
      sudo apt update
      sudo apt install -y neofetch
      ;;
    pacman)
      echo "Instalando Neofetch via pacman..."
      sudo pacman -Sy --noconfirm neofetch
      ;;
    dnf)
      echo "Instalando Neofetch via dnf..."
      sudo dnf install -y neofetch
      ;;
    zypper)
      echo "Instalando Neofetch via zypper..."
      sudo zypper install -y neofetch
      ;;
    *)
      echo "Gerenciador de pacotes não suportado. Instale Neofetch manualmente."
      exit 1
      ;;
  esac
}
